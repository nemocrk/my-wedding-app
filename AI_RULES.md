...
2. **Tracciatura**: Dopo aver analizzato il codice crea il numero necessario di ISSUE Github con l'analisi tecnica delle implementazioni
3. **Branch Strategy**: Verifica se esiste un branch dedicato all'attività ed utilizza quello. Qualora non vi fosse, creane uno nuovo ad HOC.
4. **No-Rewrite**: Non riscrivere file da zero se non strettamente necessario. Applicare patch/diff.
5. **Testing**: Ogni feature deve essere verificata (es. `curl`, unit test, smoke test).
6. **Sicurezza**: Mai esporre il DB su Internet. Validare sempre gli input API.
7. **Aggiornamento**: Prima di aggiungere nuove dipendenze, assicurati sui vari package manager (es. NPMjs, ...) che stai utilizzando l'ultima versione stabile del componente.
8. **Conferma**: Dopo aver concluso le modifica aggiorna le ISSUE Github con lo stato di avanzamento
...
3. **Analisi Diffs**: Dopo la creazione di una PR, bisogna analizzare tutte le diffs, elencare i file modificati, aggiunti o eliminati, indicare il numero di linee modificate e motivarle.
